...
public class AverageTemperature5 extends Configured implements Tool {
	...
	public static class AverageTemperatureMapper extends
		...
		protected void cleanup(Context context) throws IOException,
			...
			for (Map.Entry<Text, Pair> e : map.entrySet()) {
				context.write(
						new Key(new IntWritable(Integer.parseInt(e.getKey()
								.toString()))), e.getValue());
			}
		}
	}
	...
	public static class AverageTemperaturePartitioner extends Partitioner<Key, Pair> {
		public int getPartition(Key key, Pair pair, int numReduceTasks) {
			if (numReduceTasks == 2) {
				if (key.getYear().get() < 1930) {
					return 0;
				} else {
					return 1;
				}
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(conf, new AverageTemperature5(), args);
	...
	public int run(String[] args) throws Exception {
		...
		job.setJarByClass(AverageTemperature5.class);
